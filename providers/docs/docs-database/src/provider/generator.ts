...
import { Index, isDatabaseService } from '@ez4/database/library';
...
const getColumnsOutput = (columns: ObjectSchemaProperties, relations: TableRelation[], indexes: TableIndex[]) => {
  const output = [];
...
const getRelationsOutput = (targetTable: string, tables: DatabaseTable[], columns: ObjectSchemaProperties, relations: TableRelation[]) => {
  const output = [];
