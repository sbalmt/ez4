...
describe('ws message metadata errors', () => {
  registerTriggers();

  it('assert :: incomplete service message', () => {
    const [error1, error2] = parseFile('incomplete-message', 2);

    ok(error1 instanceof IncompleteTargetError);
    deepEqual(error1.properties, ['handler']);

    ok(error2 instanceof IncompleteServiceError);
    deepEqual(error2.properties, ['message']);
  });

  it('assert :: invalid message property', () => {
    const [error1] = parseFile('invalid-message-property', 1);
