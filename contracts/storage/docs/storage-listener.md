...
- `Begin` and `Ready` are fail-fast. If either listener event fails, object-event processing fails.
...
Listeners should keep best-effort events focused on observability, such as logging, metrics, and tracing. They should not perform required object processing in those events.
