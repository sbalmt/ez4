  ...
  it('assert :: prepare update secondary relation (select new connection)', async ({ assert }) => {
    ...
    assert.equal(
      ...
      `WITH ` +
        ...
        // Update and return
        `UPDATE ONLY "ez4_test_table" AS "U" SET "id" = :0, "primary_id" = :1 FROM "Q0" RETURNING "Q0".*`
    );
  ...
  it('assert :: prepare update secondary relation (multiple connections and select)', async ({ assert }) => {
    ...
    assert.equal(
      ...
      `WITH ` +
        ...
        // Main record
        `"Q1" AS (UPDATE ONLY "ez4_test_table" AS "U" SET "id" = :0, "primary_2_id" = :1 FROM "Q0" RETURNING "U"."id"), ` +
        // First relation
